   ...
   def  get_market_02(fecha1,fecha2) 
     ...
     facturas = Sellvale.find_by_sql(['Select sellvales.cod_tar,
      ...
      return facturas 
       ...
       def get_pendientes_cliente(fecha1,fecha2,cliente)
          ...
          @facturas = Factura.where([" balance > 0  and  company_id = ? AND fecha >= ? and fecha<= ? and customer_id = ? and division_id = 2 ", self.id, "#{fecha1} 00:00:00","#{fecha2} 23:59:59", cliente ]).order(:customer_id,:moneda_id,:fecha)
          return @facturas
    ...
    def get_facturas_day_value_cliente(fecha1,fecha2,cliente,value = "total",moneda)
      ...
      def get_pendientes_day_customer_detraccion(fecha1,fecha2,cliente)
       ...
       def get_purchases_year_month_value( year,month, value = "total_amount")
        ...
        def get_purchases_day_tipo(fecha1,fecha2,tipo)
          if tipo =="2" 
            @purchases = Purchase.where([" company_id = ? AND date2 >= ? and date2 <= ? and processed = ? and gasto_id <> ? ", self.id, "#{fecha1} 00:00:00","#{fecha2} 23:59:59","1","30"]).order(:supplier_id,:moneda_id,:date1)    
          else   
            @purchases = Purchase.where([" company_id = ? AND date2 >= ? and date2 <= ?  AND tipo = ?  and processed = ? and gasto_id <> ?", self.id, "#{fecha1} 00:00:00","#{fecha2} 23:59:59", tipo, "1","30"]).order(:supplier_id,:moneda_id,:date1)
          end 
          return @purchases 
        end
        def get_purchases_day_tipo2(fecha1,fecha2,tipo,proveedor)
          ...
          if tipo =="2" 
            @purchases = Purchase.where([" company_id = ? AND date2 >= ? and date2 <= ? and processed = ? and supplier_id = ? and gasto_id <> ?", self.id, "#{fecha1} 00:00:00","#{fecha2} 23:59:59","1",proveedor ,"30"]).order(:supplier_id,:moneda_id,:date1)    
          else   
            @purchases = Purchase.where([" company_id = ? AND date2 >= ? and date2 <= ?  AND tipo = ?  and processed = ? and supplier_id = ? and gasto_id<>?", self.id, "#{fecha1} 00:00:00","#{fecha2} 23:59:59", tipo, "1",proveedor,"30"]).order(:supplier_id,:moneda_id,:date1)
          end 
          return @purchases 
        end
      ...
      # ajustes de inventarios
       ...
       ###INVENTARIO  STOCKS MARKET



## INVENTARIO POR LOCAL 
 def  get_stocks_inventarios_market(fecha1,fecha2,product1,estado,local)
    ...
    ##cambie solo a julio 
    fecha_inventario = "2022-05-30"

     @inv = Inven.where('fecha_a >=?  and fecha_a < ?',"#{fecha_inventario} 00:00:00","#{fecha1} 00:00:00" )  

    
    
        for invdetail in @inv


              producto_code = Product.find_by(:code => invdetail.cod_prod )

      puts invdetail.cod_prod 

          if producto_code == "0000000000089"

            producto_code= "0983"
          end  

           movdetail  = MovementDetail.find_by(:product_id=>producto_code)  



          if movdetail

            @cantidad =  invdetail.stk_fisico - invdetail.stk_act 

            if @cantidad == nil
            movdetail.stock_inicial += 0   
            else
            movdetail.stock_inicial += @cantidad 
            end

            if invdetail.costo == nil
              movdetail.price = 0  
           else 
                movdetail.price = invdetail.costo 
      ...
      #ingresos
     @ingdetail = CompraMarket.where('fecha > ? and fecha  <  ? ',"#{fecha_inventario} 23:59:59","#{fecha1} 00:00:00" )

        for detail in @ingdetail 

           $lcFecha = detail.fecha.to_date
          ...
          $lcPreciosinigv = detail.preciosigv
          ...
          if movdetail
            if detail.cantidad == nil
              movdetail.stock_inicial += 0   
            else 
              movdetail.stock_inicial +=  detail.cantidad 
            end 

            if detail.preciosigv == nil
             movdetail.price = 0 
            else
              if $lcmoneda != nil                 
                if $lcmoneda == 2
                  movdetail.price = detail.preciosigv
     ...
     #salidas 
    @saldetail  = Market.where('fecha>? and fecha <  ?',"#{fecha_inventario} 23:59:59","#{fecha1} 00:00:00" )
   ...
   #actualiza ajustes de inventarios
     ...
     @ajuste  = Ajust.where('fecha1>? and fecha1 <  ? ',"#{fecha_inventario} 23:59:59","#{fecha1} 00:00:00" )
 ...
 ## Aca calcula saldos con el detalle de las ventas y compras    
  #actualiza  el costo de la salida
     @invdetail  = Inven.where('fecha_a >= ? and  fecha_a <= ? ',"#{fecha1} 00:00:00","#{fecha2} 23:59:59")  
      ...
      #ingresos0 
     @ingdetail = CompraMarket.where('fecha >= ? and fecha  <= ? ',"#{fecha1} 00:00:00","#{fecha2} 23:59:59")
        ...
        for detail in @ingdetail 
          ...
          $lcPreciosinigv = detail.preciosigv
           ...
           producto_code = Product.find_by(:code => detail.cod_prod )
          ...
          if movdetail
            if detail.cantidad  == nil 
              ...
              movdetail.ingreso += detail.cantidad 
            end 
     ...
     #salidas 
    @saldetail  = Market.where('fecha>= ? and fecha <= ? ',"#{fecha1} 00:00:00","#{fecha2} 23:59:59")

      
     
        for detail in @saldetail 

             $lcFecha = detail.fecha 
         ...
         producto_code = Product.find_by(:code => detail.cod_prod )
          ...
          if movdetail
            ...
            if detail.cantidad  == nil
              ...
              movdetail.salida += detail.cantidad 
            end
              movdetail.save           
          else     
...
# ajustes de inventarios
    ...
    #@inv = MovementDetail.joins(:product).order("movement_details.id")
    ...
    return @inv 
      ...
      def get_purchases_pendientes_day_value(fecha1,fecha2,value = "total_amount",cliente,moneda)
       ...
       def  get_parte_10(fecha1)
          @varilla = Varillaje.select("tanque_id").where(["fecha >= ? and fecha <= ? ", "#{fecha1} 00:00:00","#{fecha1} 23:59:59"  ]).group("tanque_id")
          return @varilla 
       end 
 ...
 def  get_parte_4_vale_total(fecha1,fecha2) 
   ...
   if tarjeta  == 7
      ...
      def get_facturas_by_day_value2(fecha1,fecha2,moneda,value='total')
        ...
        def  get_ventas_combustibles1(fecha1,fecha2) 
