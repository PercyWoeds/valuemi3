...
def get_purchases_pendientes_day_value(fecha1,fecha2,value = "total_amount",cliente,moneda)
 ...
 def  get_parte_4(fecha1,fecha2) 
     @contado = Sellvale.where(["fecha >= ? and fecha <= ? and sellvales.tipo = ?  and sellvales.td= ?", "#{fecha1} 00:00:00","#{fecha2} 23:59:59" ,"1","N" ]).order(:cod_cli,:fecha,:cod_prod).joins("LEFT OUTER JOIN customers ON sellvales.cod_cli = customers.account AND customers.tipo = '2' ")
    return @contado
 end 
