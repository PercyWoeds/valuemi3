...
# Export serviceorder to PDF
  def rpt_salidas4_all_pdf

    @company=Company.find(params[:id])          
    @fecha1 = params[:fecha1]    
    @fecha2 = params[:fecha2]    
    @categoria = params[:products_category_id]    
    @placa = params[:ac_item_id]    
    @categoriaCheck = params[:categoriaCheck]    
    
    puts "categoria "
    puts @categoriaCheck
    
    if @categoriaCheck == 'on'     
     @facturas_rpt = @company.get_salidas_day3_2(@fecha1,@fecha2,@placa)  
     @namecategoria = "Todas las categorias "
    else
      @namecategoria = @company.get_categoria_name(@categoria)      
      @facturas_rpt = @company.get_salidas_day3_1(@fecha1,@fecha2,@categoria,@placa)
    end 
    Prawn::Document.generate("app/pdf_output/rpt_factura.pdf") do |pdf|
        pdf.font "Helvetica"
